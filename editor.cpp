...
void Editor::update(Input* input, Scene* scene, Configuration* config)
{
    ...
    if (input->is_button_state(Input::Button::HELP_DISPLAY_INFORMATIONS, Input::Button_State::PRESSED))
    {
    ...
    if (input->is_button_state(Input::Button::DEBUG_HIDE_IDS, Input::Button_State::PRESSED))
    {
        config->should_hide_ids = !config->should_hide_ids;
    }
}
