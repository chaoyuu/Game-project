...
void Player::simulate_AI(Uint32, Assets* assets, Input* input,Scene*)
{
	...
	case State::Idle:
		...
		else if (input->is_button_state(Input::Button::ATTACKING, Input::Button_State::PRESSED))
		{
			push_state(State::Attacking, assets);
		}
		else if (input->is_button_state(Input::Button::DIEING, Input::Button_State::PRESSED))
		{
	...
	case State::Running:
		...
		else if (input->is_button_state(Input::Button::ATTACKING, Input::Button_State::DOWN))
		{
			push_state(State::Attacking, assets);
		}
		break;
	case State::Attacking:
		if (input->is_button_state(Input::Button::ATTACKING, Input::Button_State::RELEASED))
		{
...
void Player::handle_enter_state(State state, Assets* assets)
{
	...
	case State::Attacking:
	{
		_texture_id = "Texture.Player.Attacking";
		_speed = 1.5f;
		const int attacking_channel = 3;
		Sound* sound = (Sound*)assets->get_asset("Sound.Attacking");
		Mix_PlayChannel(attacking_channel, sound->data(), -3);
	...
	case State::Dieing:
	{
		_texture_id = "Texture.Player.Dieing";
		_speed = 1.3f;
		const int dieing_channel = 4;
		Sound* sound = (Sound*)assets->get_asset("Sound.Dieing");
		Mix_PlayChannel(dieing_channel, sound->data(), -4);
		break;
	}
	}
}
...
void Player::handle_exit_state(State state, Assets*)
{
	...
	case State::Dieing:
	{const int dieing_channel = 4;
	Mix_HaltChannel(dieing_channel);
	break;
	}
